...
aws s3api put-object-tagging --bucket ${BUCKET} --key vms/$VM_NAME/${OVA_NAME} --tagging "TagSet=[{Key='Filedrop_UI',Value=${FILEDROP_UI}},{Key='K8S_Flavor',Value='K3S'},{Key='CS_K8S_API',Value=${CS_API}},{Key='ICAP_FLAVOUR',Value=${ICAP_FLAVOUR}},{Key='M_UI',Value=${INSTALL_M_UI}}]"